    ...
    def update(self, **kwargs):
        for k, v in kwargs.items():
            ...
            if isinstance(v, torch.Tensor):
                ...
                continue
            self.meters[k].update(v) #2025/11/02
...
def mask_iou(pred_label, label):
    pred_bin = (pred_label > 0)[0].int()
    gt_bin   = (label > 128)[0].int()

    inter = ((gt_bin * pred_bin) > 0).sum().float()
    union = ((gt_bin + pred_bin) > 0).sum().float()

    if union == 0:
        return torch.tensor(1.0, device=pred_label.device)
    return inter / union #2025/11/03
...
def boundary_iou(gt, dt, dilation_ratio=0.02):
    ...
    gt_b = mask_to_boundary(gt_np, dilation_ratio)
    dt_b = mask_to_boundary(dt_np, dilation_ratio)
    inter = ((gt_b * dt_b) > 0).sum()
    union = ((gt_b + dt_b) > 0).sum()
    if union == 0:
        return torch.tensor(1.0, device=device).float()
    return torch.tensor(inter / union, device=device).float() #2025/11/03 iou nan modified